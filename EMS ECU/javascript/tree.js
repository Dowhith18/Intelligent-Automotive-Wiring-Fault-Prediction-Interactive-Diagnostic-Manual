...
function showPath(win, path, showquery, showanchor) {
  ...
  if (doc) {
    ...
    if (node) scrollIntoView(win, node);
  ...
  if ("server" == agent) {
    ...
    if (liNodes) {
      for (var i = 0; i < liNodes.length; i++) {
        var node = liNodes[i];
        win.closeNode(node);
        if (node.className == "open") node.className = "closed";
...
function getVerticalScroll(win, node) {
  ...
  var scroll = 0;
  if (nodeTop >= pageTop) {
    if (nodeBottom <= pageBottom) scroll = 0; // already in view
    else scroll = nodeBottom - pageBottom;
  } else {
...
function showCurrentDocument() {
  ...
  if (frameWin.location.hash) sPath += frameWin.location.hash;
...
function jumpToAdjascent(next) {
  ...
  if (!path) return;
  if (frameWin.location.hash) path += frameWin.location.hash;
