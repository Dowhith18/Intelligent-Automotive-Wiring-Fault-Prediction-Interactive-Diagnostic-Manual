...
function getWindowHeight(win) {
  ...
  if (typeof win.innerWidth == "number") {
    ...
    win.document.body &&
    (win.document.body.clientWidth || windocument.body.clientHeight)
  ) {
    //IE 4 compatible
...
function getWindowWidth(win) {
  ...
  if (typeof win.innerWidth == "number") {
    ...
    win.document.body &&
    (win.document.body.clientWidth || win.document.body.clientHeight)
  ) {
    //IE 4 compatible
...
function getFirstLevelChildByElementName(elm, name) {
  ...
  if (elm.tagName) {
    if (name == elm.tagName.toLowerCase()) return elm;
  }
  elm = elm.firstChild;
...
function setStartPage() {
  ...
  if (startAt) {
    ...
    if (frameWin) {
      ...
      if (startHash) startAt += "#" + startHash;
