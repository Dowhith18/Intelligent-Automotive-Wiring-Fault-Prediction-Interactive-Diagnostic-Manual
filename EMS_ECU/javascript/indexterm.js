...
function opendoc(e) {
  ...
  if (el) {
    if (alink.hash == "#_see_also_") {
      ...
      if (idref) findReference(idref);
    } else el.src = alink.className;
  }
}
...
function search(e, str) {
  ...
  for (var i = 0; i < INDEX_ARRAY.length; i++) {
    ...
    if (!title || title.length == 0 || title.length < str.length) continue;
...
function removeElementId(id) {
  ...
  if (elm) {
    ...
    } else elm.removeAttribute("id");
  }
}

function navigate(e, navigate) {
  if (navigate != 38 && navigate != 40) return;
  ...
  if (navigate == 38) {
    ...
    if (!elm) {
      ...
      for (var i = nodes.length - 1; i >= 0; i--) {
        ...
        if ("item" != node.className && "folder" != node.className) continue;
...
function getContainingLI(elm) {
  if (!elm) return null;
  while (elm) {
    if (elm.tagName && "LI" == elm.tagName.toUpperCase()) return elm;
    elm = elm.parentNode;
  }
  return null;
}
...
function getLastChildLI(elm) {
  ...
  if (nodes.length > 0) return nodes.item(nodes.length - 1);
...
function getNextParentLI(elm) {
  ...
  while (elm) {
    if (elm.tagName && "LI" == elm.tagName.toUpperCase()) break;
    elm = elm.parentNode;
  }
...
function _showInInputBox(elem) {
  ...
  if (tabs) content = getFrameContentDocument(tabs);
...
function initIndexTerm() {
  ...
  for (var i = 0; i < document.links.length; i++) {
    ...
    if (!document.links[i].ondblclick) document.links[i].ondblclick = linkOver;
  }
...
function initIndexSearch() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if ("primary_index" != elm.className) continue;
